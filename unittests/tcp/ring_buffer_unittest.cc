...
TEST(RingBuffer, GetWritePositionsAndCommitWrite) {
  ...
  ASSERT_EQ((size_t)0, rb.Size());
  ASSERT_EQ((size_t)0, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());
  struct iovec write_positions[2];
  rb.GetWritePositions(write_positions, 2);
  ASSERT_EQ((size_t)10, write_positions[0].iov_len);
  ASSERT_EQ((size_t)0, write_positions[1].iov_len);
  ::memcpy(write_positions[0].iov_base, "abc", 3);
  rb.CommitWrite(3);
  ASSERT_EQ((size_t)3, rb.Size());
  ASSERT_EQ((size_t)3, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());

  write_positions[0].iov_len = 0;
  write_positions[1].iov_len = 0;
  rb.GetWritePositions(write_positions, 2);
  ASSERT_EQ((size_t)7, write_positions[0].iov_len);
  ASSERT_EQ((size_t)0, write_positions[1].iov_len);
  ::memcpy(write_positions[0].iov_base, "abc", 3);
  rb.CommitWrite(3);
  ASSERT_EQ((size_t)6, rb.Size());
  ASSERT_EQ((size_t)6, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());

  write_positions[0].iov_len = 0;
  write_positions[1].iov_len = 0;
  rb.GetWritePositions(write_positions, 2);
  ASSERT_EQ((size_t)4, write_positions[0].iov_len);
  ASSERT_EQ((size_t)0, write_positions[1].iov_len);
  ::memcpy(write_positions[0].iov_base, "abcd", 4);
  rb.CommitWrite(4);
  ASSERT_EQ((size_t)10, rb.Size());
  ASSERT_EQ((size_t)10, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());

  write_positions[0].iov_len = 0;
  write_positions[1].iov_len = 0;
  rb.GetWritePositions(write_positions, 2);
  ASSERT_EQ((size_t)0, write_positions[0].iov_len);
  ASSERT_EQ((size_t)0, write_positions[1].iov_len);
}
...
TEST(RingBuffer, GetReadPositionsAndCommitRead) {
  ...
  ASSERT_EQ((size_t)10, read_positions[0].iov_len);
  ASSERT_EQ((size_t)0, read_positions[1].iov_len);
  ...
  rb.CommitWrite(5);
  ASSERT_EQ((size_t)5, rb.Size());
  ASSERT_EQ((size_t)5, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());

  read_positions[0].iov_len = 0;
  read_positions[1].iov_len = 0;
  rb.GetReadPositions(read_positions, 2);
  ASSERT_EQ((size_t)5, read_positions[0].iov_len);
  ASSERT_EQ((size_t)0, read_positions[1].iov_len);
  ...
  write_positions[1].iov_len = 0;
  rb.GetWritePositions(write_positions, 2);
  ASSERT_EQ((size_t)5, write_positions[0].iov_len);
  ASSERT_EQ((size_t)5, write_positions[1].iov_len);
  ::memcpy(write_positions[0].iov_base, "abcde", 5);
  ...
  rb.GetReadPositions(read_positions, 2);
  ASSERT_EQ((size_t)5, read_positions[0].iov_len);
  ASSERT_EQ((size_t)1, read_positions[1].iov_len);
  ASSERT_EQ(0, ::memcmp(read_positions[0].iov_base, "abcde", 5));
...
TEST(RingBuffer, WriteAndRead) {
  ...
  ASSERT_EQ("ab", buffer);
  ASSERT_EQ((size_t)8, rb.Size());
  ASSERT_EQ((size_t)8, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());
  ...
  data.set("abcdef", 6);
  ASSERT_TRUE(rb.Write(data));
  ASSERT_EQ((size_t)9, rb.Size());
  ASSERT_EQ((size_t)9, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());

  ASSERT_TRUE(rb.Read(&buffer, 5));
  ASSERT_EQ("abcdeabcdeab", buffer);
  ASSERT_EQ((size_t)4, rb.Size());
  ASSERT_EQ((size_t)4, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());
...
TEST(RingBuffer, FindStringAndReform1) {
  ...
  data.set("abc", 3);
  ASSERT_TRUE(rb.Write(data));
  ASSERT_EQ((size_t)7, rb.Size());
  ASSERT_EQ((size_t)7, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());
...
TEST(RingBuffer, FindStringAndReform3) {
  ...
  ASSERT_EQ((size_t)2, rb.Size());
  ASSERT_EQ((size_t)2, rb.Length());
  ASSERT_EQ((size_t)10, rb.Capacity());
...
TEST(RingBuffer, FindCharAndReform4) {
  ...
  ASSERT_TRUE(rb.Write(data));
  ASSERT_EQ((size_t)17, rb.Size());
  ASSERT_EQ((size_t)17, rb.Length());
  ASSERT_EQ((size_t)500, rb.Capacity());
...
TEST(RingBuffer, Stats) {
  {
    cnetpp::tcp::RingBuffer rb(50);
    ASSERT_EQ(50, cnetpp::tcp::RingBuffer::TotalMemory());
    ASSERT_EQ(500, cnetpp::tcp::RingBuffer::MaxRingBuffer());
    ASSERT_EQ(10, cnetpp::tcp::RingBuffer::MinRingBuffer());
  }
  ASSERT_EQ(0, cnetpp::tcp::RingBuffer::TotalMemory());
  ASSERT_EQ(500, cnetpp::tcp::RingBuffer::MaxRingBuffer());
  ASSERT_EQ(10, cnetpp::tcp::RingBuffer::MinRingBuffer());
}
